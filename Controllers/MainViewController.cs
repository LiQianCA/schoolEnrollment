...
namespace schoolEnrollment.Controllers
{
    public class MainViewController : Controller
    {
        ...
        public string GetStudentTableData()
        {
            ...
            try
            {
                var result = from ul in dataContext.Users
                             
                             where ul.Role_Id!=1 &&  (string.IsNullOrEmpty(HttpContext.Request.Query["user_name"].ToString()) ? 1 == 1 : ul.Name.Contains(HttpContext.Request.Query["user_name"].ToString()))
                             select new
                             {
                                 ul.User_Id,
                                 ul.User_Account,
        ...
        public string DeleteUser(string id)
        {
            ...
            {
                resData["state"] = -1;
                resData["msg"] = "User_Id can not be empty!";
            }
            else
            {
                int idtmp = -1;
                if (id != "")
                {
                    idtmp = Convert.ToInt32(id);
                }

                Users author = dataContext.Set<Users>().Where(p => p.User_Id == idtmp).FirstOrDefault();
                if (author != null)
                {
                    dataContext.Set<Users>().Remove(author);
                    dataContext.SaveChanges();
